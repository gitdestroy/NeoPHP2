...
class SQLDataTable
{
    ...
    public function addWhere ($column, $operator, $value, $connector = SQL::OPERATOR_AND)
    {
        ...
        $this->sql->whereClause->addCondition($column, $operator, $value, $connector);
        return $this;
    }
    
    public function addWhereGroup (SQLConditionGroup $conditionClause, $connector = SQL::OPERATOR_AND)
    {
        if (empty($this->sql->whereClause))
            $this->sql->whereClause = new SQLConditionGroup();
        $this->sql->whereClause->addConditionClause($conditionClause, $connector);
        return $this;
    }
    ...
    public function addRawWhere ($expression, array $bindings = [], $connector = SQL::OPERATOR_AND)
    {
        if (empty($this->sql->whereClause))
            $this->sql->whereClause = new SQLConditionGroup();
        $this->sql->whereClause->addRawCondition($expression, $bindings, $connector);
        return $this;
    }
    ...
    public function addHaving ($column, $operator, $value, $connector = SQL::OPERATOR_AND)
    {
        ...
        $this->sql->havingClause->addCondition($column, $operator, $value, $connector);
        return $this;
    }
    ...
    public function addHavingGroup (SQLConditionGroup $conditionClause, $connector = SQL::OPERATOR_AND)
    {
        if (empty($this->sql->havingClause))
            $this->sql->havingClause = new SQLConditionGroup();
        $this->sql->havingClause->addConditionClause($conditionClause, $connector);
        return $this;
    }
    ...
    public function addRawHaving ($expression, array $bindings = [], $connector = SQL::OPERATOR_AND)
    {
        if (empty($this->sql->havingClause))
            $this->sql->havingClause = new SQLConditionGroup();
        $this->sql->havingClause->addRawCondition($expression, $bindings, $connector);
        return $this;
    }
    ...
    public function get (ReflectionAnnotatedClass $entityClass = null)
    {
        ...
        do
        {
            ...
            if ($result != false)
            {
                $results[] = ($entityClass != null)? ConnectionUtils::createEntity($entityClass, $result) : $result;
            }
        }
        while (!empty($result));
        return $results;
    ...
    private function getConditionGroupExpression (SQLConditionGroup $conditionGroup, array &$bindings)
    {
        ...
        foreach ($conditionGroup->getConditions() as $condition)
        {
            ...
            else if (isset($condition["group"]))
            {
                $expression .= "(" . $this->getConditionGroupExpression($condition["group"], $bindings) . ")";
            }
            else
            {
